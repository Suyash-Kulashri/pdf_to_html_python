...
try:
    ...
    if tables.n == 0:
        ...
        for table_idx, table in enumerate(tables, start=1):
            ...
            # Export each table as an HTML file
            html_file = os.path.join(output_dir, f"table_{table_idx}_page_{table.page}.html")
            table.to_html(html_file)
            print(f"Saved table {table_idx} to {html_file}")
        
        # Optionally, export all tables as a zip file containing HTML files
        zip_file = os.path.join(output_dir, "all_tables.zip")
        tables.export(os.path.join(output_dir, "all_tables.html"), f='html', compress=True)
